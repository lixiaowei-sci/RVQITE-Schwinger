...
"""For Fig.4(a): 
x: q\in[0,1]  vs. y: mu\in[-4,4]; m=1 """
N=10
J=0.5
w=0.5
m=1
circ = hva
obsever = np.zeros((51,51,4))   # u(1): obsever[:,:,0]; chiral: obsever[:,:,1]; electric: obsever[:,:,2]; entropy: obsever[:,:,3] 
iter_q =0
for q in np.linspace(-1,1,51):
    iter_m =0
    for mu in np.linspace(-4,4,51):
