...
def exact_evolution(N,J,w,m,mu,q,Q):
    ...
    for i in range(1024):
        ...
        if abs(charge-Q)<1e-5:
            energy1 = eigenvalues[i]
            print('test', i, 'charge', charge, 'energy', eigenvalues[i])
            break 
    for i in range(1024):
        charge = eigenvectors[:,i].conj().T@Q_ops@eigenvectors[:,i]
        
        if abs(charge-Q-1)<1e-5:
            energy2 = eigenvalues[i]
            print('test', i, 'charge', charge, 'energy', eigenvalues[i])
            break 
    return energy2-energy1    


def EOS(N,J,w,m,mu,q,Q):
    E = exact_evolution(N,J,w,m,0,q,Q)
...
#         def fun(mu):
#             return EOS(N,J,w,m,mu,q,Q)
#         mu0, mu1=-4, 4
#         mu = bisection_method(fun,mu0,mu1)
#         if mu is None:
#             pass 
#         else:
#             x.append(q)
...
#     file.write(str(phasex))
#     file.write(str(phasey))
...
# x=[]
# y=[]
# for q in np.linspace(0.6,0,10):
#     def fun(m):
#         return EOS(N,J,w,m,mu,q,Q)
#     m0, m1=-4,-0.7 
#     m = bisection_method(fun,m0,m1)
#     if m is None:
#         pass 
#     else:
#         x.append(q)
...
with open('output_mq_modify.txt', 'a+') as file:
    file.write(str(phasex_mq))
    file.write(str(phasey_mq))
